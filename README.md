...
Stratified Shuffle Split was not used in the final classifer poi_id.py code as the simplicity of of LDA and the lack of use of a parameter search did not require a more complex cross validation than a simple train test split. The classifer was fast enough that testing could be completed by directly running the tester.py script with makes use of stratified shuffle split. This method of cross validation splits the data into training and testing sets by completing randomized folds of the data while retaining a consistant percentage of each of the classifications. 
